...
class KnowledgeClient:
    ...
    def list(self) -> list[KnowledgeSummary]:
        """List all knowledge bases (sync)."""
        response = self._http_client.get(KNOWLEDGE_PATH)
        return [
            KnowledgeSummary.model_validate(kb) for kb in response.body.get('data', [])
        ]

    async def list_async(self) -> list[KnowledgeSummary]:
        """List all knowledge bases (async)."""
        response = await self._http_client.get_async(KNOWLEDGE_PATH)
        return [
            KnowledgeSummary.model_validate(kb) for kb in response.body.get('data', [])
        ]
    ...
    def add_documents(self, params: AddDocumentsParams) -> AddDocumentsResult:
        ...
        docs = [
            ...
            for d in response.body.get('documents', [])
        ]
    ...
    def list_documents(self, knowledge_id: str) -> ListDocumentsResult:
        """List documents in a knowledge base (sync)."""
        response = self._http_client.get(f'{KNOWLEDGE_PATH}/{knowledge_id}/documents')
        docs = [
            DocumentSummary.model_validate(d) for d in response.body.get('data', [])
        ]
        return ListDocumentsResult(data=docs)

    async def list_documents_async(self, knowledge_id: str) -> ListDocumentsResult:
        """List documents in a knowledge base (async)."""
        response = await self._http_client.get_async(
            f'{KNOWLEDGE_PATH}/{knowledge_id}/documents'
        )
        docs = [
            DocumentSummary.model_validate(d) for d in response.body.get('data', [])
        ]
